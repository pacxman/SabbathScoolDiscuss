...
//        mSwipeListener = listener;
    ...
    public RecyclerTouchListener setSwappable(int foregroundID, int backgroundID, OnSwipeOptionsClickListener listener) {
        this.swipeable = true;
    ...
    public RecyclerTouchListener setSwappable(boolean value) {
    ...
    public RecyclerTouchListener setUnSwappableRows(Integer... rows) {
        this.unSwappableRows = new ArrayList<>(Arrays.asList(rows));
        return this;
    }
    ...
    public void openSwipeOptions(int position) {
        if (!swipeable || rView.getChildAt(position) == null
                || unSwappableRows.contains(position) || shouldIgnoreAction(position))
            return;
        if (bgWidth < 2) {
...
//                        bgView.getLayoutParams().height = fgView.getHeight();
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (swipeable && !swipedRight && swipedLeftProper && touchedPosition != RecyclerView.NO_POSITION
                        && !unSwappableRows.contains(touchedPosition) && !bgVisible) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swipeable && isFgSwiping && !unSwappableRows.contains(touchedPosition)) {
                    if (bgView == null) {
                        bgView = touchedView.findViewById(bgViewID);
...
//                        }
                ...
                else if (swipeable && isFgSwiping && unSwappableRows.contains(touchedPosition)) {
                    if (deltaX < touchSlop && !bgVisible) {
                        float translateAmount = deltaX - mSwipingSlop;
                        if (bgView == null)
                            bgView = touchedView.findViewById(bgViewID);
